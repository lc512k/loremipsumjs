...
$(function() {
  'use strict';
  var $loremDivs, i, loremStrTemp, Util;

  //TEMP move to separate file
  ...
  $loremDivs = $('[data-lorem]');

  console.log('$loremDivs', $loremDivs);
  ...
  Util = {
    ...
    splitSliceJoin: function(segments, start, splitStr, joinStr, maxSegmentLength) {
      ...
      var splits = loremStrTemp.split(splitStr);
      if (maxSegmentLength) {
  ...
  for (i = 0; i < $loremDivs.length; i++) {
    ...
    if (n > 0) {
      ...
      result = '';
      console.warn('DEBUG: invalid class', loremType, n);
    }
    div.html(result);
  }
});
